...
export class EegComponent implements OnInit {
      ...
      for (const [label, map] of this.labelsSignal.entries()) {
        if (map.has(this.initial + 1) && map.has(end)) {
          ...
          if (channels.length > 50) {
            bufferInitial += Math.floor(
              this.window_size * this.signalsInSecond
            );
            bufferEnd =
              bufferInitial +
              Math.floor(this.window_size * this.signalsInSecond);
          } else if (channels.length > 25 && channels.length < 50) {
            bufferInitial +=
              2 * Math.floor(this.window_size * this.signalsInSecond);
            bufferEnd =
              bufferInitial +
              Math.floor(this.window_size * this.signalsInSecond) * 2;
          } else if (channels.length <= 25) {
            bufferInitial +=
              4 * Math.floor(this.window_size * this.signalsInSecond);
            bufferEnd =
              bufferInitial +
              Math.floor(this.window_size * this.signalsInSecond) * 4;
          }
  ...
  getBackendData(initial: number, end: number, channels: any[]) {
    this.services
      .getDataAboutLabel(this.id, channels, this.token, initial, end)
      .subscribe((channelsMap) => {
        this.endLimit = end;
  ...
  right() {
    ...
    if (
      this.initial >
      this.indices - Math.floor(this.window_size * this.signalsInSecond)
    ) {
      this.initial =
        this.indices - Math.floor(this.window_size * this.signalsInSecond);
    }
